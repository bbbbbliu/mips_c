...
*/
uint8_t read_byte(const RAM *p, uint32_t address) {
    assert(address < p->size);
    return p->buffer[address];
}
/*
    ...
    for (size_t i = 0; i < length; i += 16) {
        ...
        for (size_t j = 0; j < 16; j++) {
            if (i + j < length) {
                ...
                fprintf(stream, "   "); // Padding spaces if we run out of data
                                        // on the last line
            }
        }
        ...
        // 3. Print the same 16 bytes as ASCII characters (helps spot strings in
        // memory)
        for (size_t j = 0; j < 16; j++) {
            if (i + j < length) {
                uint8_t byte = ram->buffer[current_addr + j];
                // isprint checks if the character can be typed/seen (letters,
                // numbers, space)
